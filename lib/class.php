...
class DBConnection {
	 ...
	 * @return boolean
	 */
	function DBConnection($db_host=HOST, $db_user=USERNAME, $db_pass=PASSWORD, $db_name=DATABASE) {
		...
		if ($this->connection){
			if (mysql_select_db($db_name, $this->connection)){
				...
				$this->display_errors('- Could not select database');
				return false;
			}
		}else{ // if we couldn't connect to the database
			$this->display_errors('- Could not connect to database');
			return false;
		}
	}
